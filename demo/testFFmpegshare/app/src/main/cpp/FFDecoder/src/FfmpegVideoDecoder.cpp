...
int FfmpegVideoDecoder::init(IProtocol *protocol, int samplerate, int channels)
{
    ...
    //查找音频流信息
    m_stream_id = av_find_best_stream(m_format_context, AVMEDIA_TYPE_VIDEO, -1, -1, NULL, 0);
    if(m_stream_id < 0)
    {
        errcode = SM_FFMPEG_DECODER_STREAM;
        goto init_exit;
    }
    LOGE("FfmpegVideoDecoder   ","m_stream_id  code: %d\n",m_stream_id);

    video_stream = m_format_context->streams[m_stream_id];
    //获取时间基，方便后面计算时间
    if(video_stream->time_base.den && video_stream->time_base.num)
    {
        m_time_base = av_q2d(video_stream->time_base);
    }
    else
    {
        errcode = SM_FFMPEG_DECODER_STREAM;
    ...
    //获取解码开始时间戳
    if(AV_NOPTS_VALUE != video_stream->start_time)
    {
        stream_start_time = video_stream->start_time * m_time_base * 1000;
    }
    //获取到解码器上下文
    m_codec_context = video_stream->codec;

    //获取解码器
    codec = avcodec_find_decoder(m_codec_context->codec_id);
    LOGE("FfmpegVideoDecoder   ","codec  : %p  %s\n",codec,codec->name);
    ...
    LOGE("FfmpegVideoDecoder   ","m_packet  : %p\n",m_packet);
    if(NULL == m_packet)
    {
        errcode = SM_FFMPEG_DECODER_NO_MEMORY;
        goto init_exit;
    ...
    LOGE("FfmpegVideoDecoder   ","m_aac_frame  : %p\n",m_video_frame);
    if(NULL == m_video_frame)
    {
        errcode = SM_FFMPEG_DECODER_NO_MEMORY;
        goto init_exit;
    }
    ...
    LOGE("FfmpegVideoDecoder   ","errcode  : %d\n",errcode);
    ...
    m_media_info.duration = video_stream->duration * m_time_base * 1000 - m_start_time + stream_start_time;
...
void FfmpegVideoDecoder::get_media_info(SMMediaInfo *mediaInfo)
{
    if (mediaInfo) {
...
int FfmpegVideoDecoder::decode(float *buf, int size, double *decode_postion, double *decode_duration)
{
...
int FfmpegVideoDecoder::seek(double pos)
{
...
void FfmpegVideoDecoder::uninit()
{
    if(m_video_frame)
    {
...
int FfmpegVideoDecoder::decode_one_frame(bool need_update_pts)
{
    int errcode = SM_FFMPEG_DECODER_SUCESS;
    int got_frame = 0;
    int decode_len = 0;
    ...
    while(true)
    {
        ...
        {
            //解码了多少的编码的数据
            decode_len = avcodec_decode_video2(m_codec_context, m_video_frame, &got_frame, m_packet);
            if(decode_len < 0)
            {
            ...
            if(got_frame)
            {
                ...
                m_final_buf = (float *)(m_video_frame->data[0]);
                m_final_size = m_video_frame->nb_samples;

                //更新当前的时间戳
                if(need_update_pts)
                {
                    m_pts = av_frame_get_best_effort_timestamp(m_video_frame) * m_time_base * 1000 - m_start_time;
                }
                else
                {
...
//文件格式上下文
    ...
    int open_code =  avformat_open_input(&pFormatCtx, filepath, NULL, &options);

    if (open_code != 0)
    {
        printf("Couldn't open input stream.\n");
        LOGE("FfmpegVideoDecoder   ","Couldn't open input stream. error code: %d >> %s\n",open_code,av_err2str(open_code));
        return -1;
    }
